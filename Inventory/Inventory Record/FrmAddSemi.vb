...
#Region "Function_Load"
    Private Sub LoadRM()
        ...
        If CmdSave.Text = "Edit" Then
            ...
            sb.AppendLine("    WHERE typecode <> '02' AND ")
            sb.AppendLine("    code NOT IN (SELECT code FROM TBLGroup WHERE code IN (SELECT CompCode FROM TBLcompound WHERE Active = '0')) ")
            sb.AppendLine("  ) xxx")
            sb.AppendLine("  WHERE Mastercode = '" & TxtCode.Text.Trim() & "' AND MRev = '" & TxtRev.Text.Trim() & "'")
            sb.AppendLine("  AND Typecode <> '01' ")
