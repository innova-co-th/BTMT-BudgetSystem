...
#Region "Set grid "
    Private Sub setGrid()
        ...
        sb.AppendLine(" FROM (")
        sb.AppendLine("   SELECT   cc.TypeCode,cc.Code,isnull(gg.Revision,'') Rev,0.00 QTY,cc.UnitBig Unit")
        sb.AppendLine("   FROM (")
        sb.AppendLine("     SELECT distinct c.Type TypeCode,c.Code,c.UnitBig ")
        sb.AppendLine("     FROM TBLConvert c")
        sb.AppendLine("     LEFT OUTER JOIN  TBLGroup g on c.Code = g.Code ")
        sb.AppendLine("   ) cc")
        sb.AppendLine("   LEFT OUTER JOIN ( ")
        sb.AppendLine("     SELECT distinct g.TypeCode, m.MasterCode, m.Revision ")
        sb.AppendLine("     FROM   TBLGroup g")
        sb.AppendLine("     LEFT OUTER JOIN TBLMASTER m on g.Code = m.MasterCode")
        sb.AppendLine("   ) gg on cc.Code = gg.MasterCode")
        sb.AppendLine("   UNION")
        sb.AppendLine("   SELECT  distinct g.TypeCode,g.Code,isnull(m.Revision,'') Rev,0.00 Qty,'KG' Unit")
        sb.AppendLine("   FROM        TBLGroup g")
        sb.AppendLine("   LEFT OUTER JOIN TBLMASTER m on g.Code = m.MasterCode ")
        sb.AppendLine(" ) Item")
